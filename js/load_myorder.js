function LoadMyOrder(){
        ...
        if (this.readyState == 4 && this.status == 200) {
            if(this.responseText){
                ...
                for(var i=0;i<data.length;i++){
                    if(document.getElementById("MyOrder").value=="All" || document.getElementById("MyOrder").value==data[i]['order_status']){
                        ...
                        idx+=1;
                        if(data[i]['order_status']=="Not Finished"){
                        ...
                        row3.innerHTML=data[i]['order_status'];
                        row4.innerHTML=data[i]['order_start_time'];
                        row5.innerHTML=data[i]["order_finish_time"];
                        row6.innerHTML=data[i]["shop_name"];
                        row7.innerHTML=data[i]["order_price"];
    ...
    xhttp.open("POST", "load_MyOrder.php", true);
